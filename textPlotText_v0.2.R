...
#' @param modelName (str) The pre-trained model name in the transformers hub.
#' @param signSubWord (str) The sign used as the split among subwords only for BERT models. The default is "##".
...
getIDsSubWord <- function(tokensTb, modelName, signSubWord = "##"){
    ...
    for (family in names(model_family$bert)){
        if (modelName %in% model_family$bert[[family]]){
            bert_found <- TRUE
            break
        }
    }
