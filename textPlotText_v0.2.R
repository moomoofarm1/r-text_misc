...
#' @noRd
embedModFam <- function(modelName){
    ...
    for (family in names(model_family)){
        if (modelName %in% model_family[[family]]){
            if (family == "BERT"){
                bert_found <- TRUE
                roberta_found <- FALSE
            }else if (family == "RoBERTa"){
                bert_found <- FALSE
                roberta_found <- TRUE
            }else{
                bert_found <- FALSE
...
getIDsSubWord <- function(tokensTb, modelName, signSubWord = "##"){
    ...
    CLSSEP <- embedModFam(modelName)
    if (CLSSEP$RoBERTa){signSubWord <- "Ġ"}
...
convertSubWord <- function(tokensTb, numSubTokens, tokenizers, modelName){
    ...
    if (!is.character(numSubTokens)){
        if (CLSSEP$BERT){
            ...
            for (rowNo in numSubTokens %>% nrow() %>% seq_len()){
                theWord <- decodeToken(tokensTb[["tokens"]][numSubTokens[[1]][rowNo]],
                  tokenizers, modelName)
...
token2Sent_rowCLSSEP <- function(aTibble, modelName){
    ...
    if (CLSSEP$BERT && CLSSEP$RoBERTa){
        print("Your input model is not supported.\n Please try bert or roberta families.")
        return (NULL)
    }

    rowCLSSEP <- which(aTibble[["tokens"]] == CLSSEP$CLS, arr.ind=TRUE) %>% tibble::as_tibble()
    rowCLSSEP_ <- which(aTibble[["tokens"]] == CLSSEP$SEP, arr.ind=TRUE) %>% tibble::as_tibble()
...
#' @param modelName The name of the used tokenizer.
#' @param include_CLS_SEP To include the embeddings of "start", "end", "both", or "none".
...
langPred <- function(predObj, theModels, lang_level = "sentence", tokenizers, modelName){
    ...
    if(lang_level == "token"){
        ...
        thePred <- predObj %>% langPred_tokens(., theModels[["modelTokens"]], tokenizers, modelName) %>% 
            tibble::as_tibble()
        return (thePred)
    }else if(lang_level == "sentence"){
...
#' @param modelName The model name in use
#' @importFrom dplyr arrange mutate
#' @importFrom furrr future_map
#' @importFrom stringr str_remove_all
#' @return The color values list contained embedObj
#' @NoRd
getLangColorTb <- function(embedObj, modelName){
    ...
    # modelName here. R internally does not support <s> & </s> for RoBERTa.
    for (modelName_ in model_family$RoBERTa){
        if (modelName == modelName_){
            coloredTb_sorted <- coloredTb %>%
              dplyr::mutate(texts = furrr::future_map(texts, 
...
getOutDf <- function(outObj, modelName){
    ...
    for (rowNo in length(posPeriod[[1]]) %>% seq_len(.)){
        if (rowNo == 1){
            ...
            if (outObj[["coloredTb"]][["texts"]][end + 1] == CLSSEP$SEP){end <- end + 1}
            outTb[["sentNo"]][start:end] <- paste0("sentNo_1")
        }else{
            start <- posPeriod[[1]][rowNo - 1] + 1
            end <- posPeriod[[1]][rowNo]
            if (outObj[["coloredTb"]][["texts"]][start] == CLSSEP$SEP){start <- start + 1}
            if (outObj[["coloredTb"]][["texts"]][end + 1] == CLSSEP$SEP){end <- end + 1}
    ...
    if ("token" %in% lang_level){
        ...
        for (rowNoSent in length(sentsUnique) %>% seq_len()){
            ...
            for (rowNoToken in length(rangeNum) %>% seq_len()){
                ...
                if (tokenSeries[["token"]][idToken][[1]] == CLSSEP$CLS || tokenSeries[["token"]][idToken][[1]] == CLSSEP$SEP){next}
    ...
    if ("all" %in% lang_level){
        ...
        if (FALSE){
            ...
            if (tokenSeries[["token"]][rowNo][[1]] == CLSSEP$CLS ||
                tokenSeries[["token"]][rowNo][[1]] == CLSSEP$SEP){next}
            tokenColor <- paste0("color:", tokenColorDf[["tokenColor"]][rowNo]
                , ";background-color:", sentColorDf[["sentColor"]][rowNo])
            token <- htmltools::span(tokenSeries[["token"]][rowNo][[1]], style=tokenColor)
            tokens[["children"]] <- append(tokens[["children"]], token %>% list())
