...
5. Uninstall R;
6. Delete the R folder and the folder in reticulate::miniconda_path() in the Library (Mac only, in the terminal, with admin privacy)
...
3. Run R code "install_miniconda(force=TRUE)";
4. Run R code: conda_create(envname="textrpp_condaenv", python_version="3.9")
5. Run R code: conda_install(envname="textrpp_condaenv", packages=c("torch"), pip=TRUE)
6. Run R code: conda_install(envname="textrpp_condaenv", packages=c("NLTK", "transformers"))
