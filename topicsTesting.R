...
#' @param method1 (string) Method to adjust p value. The default is "bonferroni".
...
topics_t_test_grouping <- function(topics_loadings, 
  ...
  t_test_func <- function(combination) {
    ...
    results <- map(2:ncol(topics_loadings), ~ list(
    ...
    names(results) <- paste0("t_", 1:(ncol(topics_loadings)-1))
